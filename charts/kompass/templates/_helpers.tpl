...
{{- range $componentName := list "vmselect" "vminsert" "vmstorage" -}}
{{- $component := index $.Values.victoriaMetricsCluster $componentName -}}
{{- if ne $component.enabled false -}}
{{- include "kompass.guardian.validateAnnotation" (dict "path" (printf "victoriaMetricsCluster.%s.annotations[guardian.kompass.zesty.co/request-ceilings]" $componentName) "annotation" (dig "annotations" "guardian.kompass.zesty.co/request-ceilings" nil $component) "expectedContainers" (list $componentName)) -}}
{{- end -}}
{{- end -}}
{{- end -}}
...
{{- end -}}
{{- if .Values.kubeStateMetrics.enabled -}}
{{- if and .Values.kubeStateMetrics.nameOverride (ne .Values.kubeStateMetrics.nameOverride "kube-state-metrics") -}}
{{- fail "kubeStateMetrics.nameOverride must be kube-state-metrics when Guardian request ceilings are enabled" -}}
{{- end -}}
...
{{- if and .Values.grafana.nameOverride (ne .Values.grafana.nameOverride "grafana") -}}
{{- fail "grafana.nameOverride must be grafana when Guardian request ceilings are enabled" -}}
{{- end -}}
{{- $grafanaExpectedContainers := list "grafana" -}}
{{- if ne .Values.grafana.sidecar.dashboards.enabled false -}}
{{- $grafanaExpectedContainers = append $grafanaExpectedContainers "grafana-sc-dashboard" -}}
{{- end -}}
{{- if ne .Values.grafana.sidecar.datasources.enabled false -}}
{{- $grafanaExpectedContainers = append $grafanaExpectedContainers "grafana-sc-datasources" -}}
{{- end -}}
{{- include "kompass.guardian.validateAnnotation" (dict "path" "grafana.annotations[guardian.kompass.zesty.co/request-ceilings]" "annotation" (dig "annotations" "guardian.kompass.zesty.co/request-ceilings" nil .Values.grafana) "expectedContainers" $grafanaExpectedContainers) -}}
{{- end -}}
...
{{- if ne (dig "cainjector" "enabled" true (index .Values "cert-manager")) false -}}
