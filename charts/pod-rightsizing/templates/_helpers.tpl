...
{{/*
Build the versioned Guardian request-ceiling annotation from chart values.
*/}}
{{- define "pod-rightsizing.guardian.requestCeilings" -}}
{{- $component := .component -}}
{{- $ceilings := required (printf "global.guardian.requestCeilings.%s is required" $component) .ceilings -}}
{{- $containers := dict -}}
{{- range $name := .containerNames -}}
{{- $container := required (printf "global.guardian.requestCeilings.%s.%s is required" $component $name) (get $ceilings $name) -}}
{{- $cpuRequest := required (printf "global.guardian.requestCeilings.%s.%s.cpuRequest is required" $component $name) (get $container "cpuRequest") -}}
{{- $memoryRequest := required (printf "global.guardian.requestCeilings.%s.%s.memoryRequest is required" $component $name) (get $container "memoryRequest") -}}
{{- $_ := set $containers $name (dict "cpuRequest" $cpuRequest "memoryRequest" $memoryRequest) -}}
{{- end -}}
{{- toJson (dict "version" 1 "containers" $containers) -}}
{{- end -}}

{{/*
Validate all required Guardian request-ceiling entries.
*/}}
{{- define "pod-rightsizing.validate.guardianRequestCeilings" -}}
{{- $requestCeilings := dig "guardian" "requestCeilings" (dict) .Values.global -}}
{{- $_ := include "pod-rightsizing.guardian.requestCeilings" (dict "component" "actionTaker" "ceilings" (get $requestCeilings "actionTaker") "containerNames" (list "kube-rbac-proxy" .Values.actionTaker.name)) -}}
{{- $_ := include "pod-rightsizing.guardian.requestCeilings" (dict "component" "recommendationsMaker" "ceilings" (get $requestCeilings "recommendationsMaker") "containerNames" (list .Values.recommendationsMaker.name)) -}}
{{- $_ := include "pod-rightsizing.guardian.requestCeilings" (dict "component" "gpuMetrics" "ceilings" (get $requestCeilings "gpuMetrics") "containerNames" (list "exporter")) -}}
{{- end -}}
